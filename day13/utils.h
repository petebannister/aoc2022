...
template <typename T>
T narrow_cast(auto&& v) {
    if (static_cast<decltype(v)>(static_cast<T>(v)) != v) {
        throw std::runtime_error("narrow_cast failure");
    }
...
inline T* map_value(std::map<K, T, C>& c, U const& key) {
    auto i = c.find(key);
    if (c.end() != i) {
        return &i->second;
    }
...
template <typename K, typename T, typename C, typename U, typename V>
inline T const map_value_or(std::map<K, T, C> const& c, U const& key, V const& value) {
    auto i = c.find(key);
    if (c.end() != i) {
        return i->second;
    }
